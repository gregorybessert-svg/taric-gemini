...
class NewImageHandler(FileSystemEventHandler):
    ...
    def _handle_path(self, path: Path) -> None:
        ...
        if result.status == "converted":
            logger.info("Konvertiert: %s", result.src_path.name)
        elif result.status == "skipped_exists":
            logger.info("Bereits vorhanden, übersprungen: %s", result.src_path.name)
        elif result.status == "skipped_unsupported":
            logger.warning("Nicht unterstützte Endung: %s", result.src_path.name)
        elif result.status == "error":
            logger.error("Fehler bei %s: %s", result.src_path.name, result.error_message)
        else:
            logger.info("Status %s für %s", result.status, result.src_path.name)
